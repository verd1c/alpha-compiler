...
func_jump_stack *init_funcJump_stack()
{
    func_jump_stack *node = malloc(sizeof(func_jump_stack));
    node->next = NULL;
...
func_jump_stack *push_func_jump(func_jump_stack *jumpFunc, unsigned quad)
{
    func_jump_stack *tmp, *new = malloc(sizeof(func_jump_stack));
...
unsigned pop_funcJump(func_jump_stack *s)
{
    if (s == NULL)
        printf("Empty Func Jump Stack\n");
    else
    {
        func_jump_stack *tmp, *prev;
        tmp = s;
        prev = tmp;
        int q = tmp->quad;
