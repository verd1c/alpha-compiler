...
expression  :   assignexpr                      {
                ...
                | expression GREATER_THAN expression   
                                                {
                                                    $$ = sym_expr(new_temp(symTable, scope));
                                                    emit_compares(IF_GREATER_I, $$, $1, $3, symTable, 0, yylineno);
                                                }
                | expression GREATER_OR_EQUAL expression    
                                                {
                                                    $$ = sym_expr(new_temp(symTable, scope));
                                                    emit_compares(IF_GREATEREQ_I, $$, $1, $3, symTable, 0, yylineno);
                                                }
                | expression LESS_THAN expression    
                                                {
                                                    $$ = sym_expr(new_temp(symTable, scope));
                                                    emit_compares(IF_LESS_I, $$, $1, $3, symTable, 0, yylineno);
                                                }
                | expression LESS_OR_EQUAL expression    
                                                {
                                                    $$ = sym_expr(new_temp(symTable, scope));
                                                    emit_compares(IF_LESSEQ_I, $$, $1, $3, symTable, 0, yylineno);
                                                }
                | expression EQUALS_EQUALS expression    
                                                {
                                                    $$ = sym_expr(new_temp(symTable, scope));
                                                    emit_compares(IF_EQ_I, $$, $1, $3, symTable, 0, yylineno);
                                                }
                | expression NOT_EQUALS expression    
                                                {
                                                    $$ = sym_expr(new_temp(symTable, scope));
                                                    emit_compares(IF_NOTEQ_I, $$, $1, $3, symTable, 0, yylineno);
                                                }
                | expression AND expression     {
                                                    $$ = sym_expr(new_temp(symTable, scope));
                                                    emit_logical(AND_I, $$, $1, $3, symTable, 0, yylineno);
                                                }
                | expression OR expression      {
                                                    $$ = sym_expr(new_temp(symTable, scope));
                                                    emit_logical(OR_I, $$, $1, $3, symTable, 0, yylineno);
                                                }
                | term  {
                            $$ = $1;
                        }
